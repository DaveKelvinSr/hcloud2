...
class Person
{
  ...
  if (!trim($this->user_name)) {
     ...
     $error = true;
  } elseif (self::getPersonIdByUser(trim($this->user_name))) {
    $error = true;
  } 
  ...
  public function addRecord() {
    ...
    if ($this->_verifyInput()) {
      ...
      // Prepare the data 
      $query = "INSERT INTO persons(first_name, last_name, gender, avatar, bloodgroup, email, phone) 
      VALUES ('" . Database::prep($this->first_name) . "',
       '" . Database::prep($this->last_name) . "',
       '" . Database::prep($this->bloodgroup) . "',
      ...
      if ($connection->query($query)) { // this inserts the row
        ...
        if ($connection->query($query)) { // this updates the row
          $return = array('', 'Your account is successfully set up.', '');   
          // add success message
          return $return;
        } else {
          // send fail message 
      ...
      $return = array('accountmaint', 'Your account is not set up. Unable to create record.', '0');
      return $return;
      }
    } else {
      // send fail message and return to contactmaint
      $return = array('accountmaint', 'Your account is not set up. Missing required information
    ...
    if ($this->_verifyInput()) {
      ...
      if (trim($_POST['password1'])) {
        ...
        $query = 'UPDATE `contacts` SET first_name=?, last_name=?, 
          gender=?, avatar=?, bloodgroup=?, email=?, phone=?, 
          user_name=?, password=?, access=? 
          WHERE id=?';
        $statement = $connection->prepare($query);
        // bind the parameters
        $statement->bind_param('ssssisssssi',$this->first_name, $this->last_name, 
          $this->gender, $this->avatar, $this->bloodgroup, $this->email, $this->phone, 
          $this->user_name, $hash_password, $this->access, 
          $this->id);  
      } else {
        // update without a password changed
        // Set up the prepared statement
        $query = 'UPDATE `persons` SET first_name=?, last_name=?,
          gender=?, avatar=?, bloodgroup=?, email=?, phone=?,
          user_name=?, access=? 
          WHERE id=?';
        $statement = $connection->prepare($query);
        // bind the parameters
        $statement->bind_param('ssssissssi',$this->first_name, $this->last_name, 
          $this->gender, $this->avatar, $this->bloodgroup, $this->email, 
          $this->phone, $this->user_name, $this->access, 
          $this->id);
      }
      ...
      if ($statement) {
        ...
        $statement->close();
        // add success message
        $return = array('', 'Your account is successfully updated.', '');
        return $return;
      } else {
        $return = array('accountmaint', 'Your account is not updated. Unable to change record.', '');
        return $return;
      }

    } else {
      // send fail message and return to categorymaint
      $return = array('accountmaint', 'Your account is not updated. Missing required information 
  ...
  public static function getRecords() {
      ...
      `user_name`, `access`
      FROM `persons` ORDER BY first_name, last_name';
  	// Run the query
  	$result_obj = '';
  	...
  	try {
  		while($result = $result_obj->fetch_object('Person')) {
  			$items[]= $result;
  		}
  ...
  public static function getPersonIdByUser($user_name) {   
    // Get the database connection
    $connection = Database::getConnection();  
    // set up the query
    $id = '';
  ...
  public static function logIn($item) {
    ...
    $id = self::getPersonIdByUser($item['user_name']);
